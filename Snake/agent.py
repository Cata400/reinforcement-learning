...
class Agent:
    ...
    def remember(self, state, action, reward, next_state, done):
        self.memory.append((state, action, reward, next_state, done))   # it pops left automatically if MAX_MEMORY is reached
    ...
    def get_action(self, state):
        ...
        self.epsilon = 100 - self.no_games
        final_move = [0, 0, 0]
...
def train():
    ...
    while True:
        ...
        if done:
            # train long memory (replay memory or experience replay), plot result
            game.reset()
