...
class ReplayBuffer:
    ...
    def store(self, sample):
        ...
        # store copies to avoid aliasing problems
        self.ss_mem[self._idx] = np.array(s, copy=True)
        # store as 1D/1x1 arrays so vstack produces consistent shapes
        self.as_mem[self._idx] = np.array([a])
        self.rs_mem[self._idx] = np.array([r], dtype=np.float32)
        self.ps_mem[self._idx] = np.array(p, copy=True)
...
class DuelingDDQN:
    ...
    def train(self, env_name, env_render_mode, seed, gamma, max_episodes, update_steps, replay_buffer, min_buffer_size, max_gradient_norm, tau):
        ...
        for episode in tqdm(range(max_episodes)):
            ...
            if (episode + 1) % 100 == 0:
                torch.save(self.online_model.state_dict(), os.path.join(self.checkpoint_dir, f'dueling_ddqn_model_{episode + 1}.pth'))

        torch.save(self.online_model.state_dict(), os.path.join(self.checkpoint_dir, 'dueling_ddqn_model_final.pth'))
        
        
    def evaluate(self, env_name, env_render_mode, n_episodes=1):
        rs = []
        env = gym.make(env_name, render_mode=env_render_mode)
        model_state_dict = torch.load(os.path.join(self.checkpoint_dir, 'dueling_ddqn_model_final.pth'))
