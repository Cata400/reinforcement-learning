...
class VPG:
    def __init__(self, policy_model_fn, policy_optimizer_fn, policy_optimizer_lr, value_model_fn, value_optimizer_fn, value_optimizer_lr):
    ...
    def interaction_step(self, state, env):
        action, log_prob, entropy = self.policy_model.full_pass(torch.tensor(state, dtype=torch.float32, device=self.device))
        value = self.value_model(torch.tensor(state, dtype=torch.float32, device=self.device))
    ...
    def train(self, env_name, env_render_mode, seed, gamma, max_episodes, entropy_loss_weight, policy_model_max_grad_norm, value_model_max_grad_norm):
        ...
        self.value_optimizer = self.value_optimizer_fn(self.value_model, lr=self.value_optimizer_lr)
        ...
        for episode in tqdm(range(max_episodes)):
            ...
            if (episode + 1) % 100 == 0:
                torch.save(self.policy_model.state_dict(), os.path.join(self.checkpoint_dir, f'vpg_model_{episode + 1}.pth'))

        torch.save(self.policy_model.state_dict(), os.path.join(self.checkpoint_dir, 'vpg_model_final.pth'))


    def evaluate(self, env_name, env_render_mode, n_episodes=1, greedy=True):
        rs = []
        env = gym.make(env_name, render_mode=env_render_mode)
        model_state_dict = torch.load(os.path.join(self.checkpoint_dir, 'vpg_model_final.pth'))
