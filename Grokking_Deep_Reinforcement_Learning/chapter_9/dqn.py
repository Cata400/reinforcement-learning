...
class EpsilonGreedyLinearStrategy:
    def __init__(self, init_epsilon=1.0, min_epsilon=0.1, decay_ratio=0.05, max_episodes=10_000):
        self.init_epsilon = init_epsilon
        self.min_epsilon = min_epsilon
        self.decay_ratio = decay_ratio
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.t = 0
        self.max_steps = decay_ratio * max_episodes
        self.epsilon = self.init_epsilon
        
    def _epsilon_update(self):
        self.epsilon = (1 - self.t / self.max_steps) * (self.init_epsilon - self.min_epsilon) + self.min_epsilon
...
class SoftmaxStrategy:
    def __init__(self, init_temperature=1.0, min_temperature=0.01, decay_ratio=0.05, max_episodes=10_000):
    ...
    def _temperature_update(self):
        self.temperature = (1 - self.t / self.max_steps) * (self.init_temperature - self.min_temperature) + self.min_temperature
        self.temperature = np.clip(self.temperature, self.min_temperature, self.init_temperature)
        self.t += 1
    ...
    def select_action(self, model, state):
        ...
        with torch.no_grad():
            ...
            scaled_qs = q_values / self.temperature
            norm_qs = scaled_qs - np.max(scaled_qs)
            exp_qs = np.exp(norm_qs)
...
class DQN:
    ...
    def train(self, env_name, env_render_mode, seed, gamma, max_episodes, update_steps, replay_buffer, min_buffer_size):
        ...
        for episode in tqdm(range(max_episodes)):
            ...
            if (episode + 1) % 100 == 0:
                torch.save(self.online_model.state_dict(), os.path.join(self.checkpoint_dir, f'dqn_model_{episode + 1}.pth'))

        torch.save(self.online_model.state_dict(), os.path.join(self.checkpoint_dir, 'dqn_model_final.pth'))
        
        
    def evaluate(self, env_name, env_render_mode, n_episodes=1):
        rs = []
        env = gym.make(env_name, render_mode=env_render_mode)
        model_state_dict = torch.load(os.path.join(self.checkpoint_dir, 'dqn_model_final.pth'))
...
if __name__ == '__main__':
    ...
    training_strategy_fn = lambda: EpsilonGreedyStrategy(epsilon=0.5)
