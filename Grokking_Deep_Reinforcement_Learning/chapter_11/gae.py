...
class GAE:
    ...
    def work(self, rank, env_name):
        ...
        while not self.get_out_signal:
            ...
            for step in count(start=1):
                ...
                if terminated or step - n_steps_start == self.max_n_steps:
                    ...
                    values.append(torch.tensor(next_value, dtype=torch.float32, device=self.device).unsqueeze(-1))
            ...
            if rank == 0:
                ...
                if (global_episode_idx + 1) % 100 == 0:
                    torch.save(self.shared_policy_model.state_dict(), os.path.join(self.checkpoint_dir, f'gae_model_{global_episode_idx + 1}.pth'))
        ...
        if rank == 0:
            torch.save(self.shared_policy_model.state_dict(), os.path.join(self.checkpoint_dir, 'gae_model_final.pth'))
    ...
    def optimize_model(self, log_probs, entropies, rewards, values, local_policy_model, local_value_model):
        ...
        gaes = torch.tensor(gaes, dtype=torch.float32, device=self.device).unsqueeze(-1)
    ...
    def train(self, env_name, env_render_mode, seed, gamma, max_episodes, entropy_loss_weight, 
            policy_model_max_grad_norm, value_model_max_grad_norm, max_n_steps, n_workers, tau):
    ...
    def evaluate(self, env_name, env_render_mode, n_episodes=1, greedy=True):
        rs = []
        env = gym.make(env_name, render_mode=env_render_mode)
        model_state_dict = torch.load(os.path.join(self.checkpoint_dir, 'gae_model_final.pth'))
